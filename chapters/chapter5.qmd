...
Crucially, we can calculate also the **sampling variability** of each effect size. The **sampling variability** is the precision of the estimated value.
...
For the examples and plots, I'm going to use simulated data [see @Gambarota2024-lp]. We simulate *unstandardized* effect sizes (UMD) because the computations are easier and the estimator is unbiased [e.g., @Viechtbauer2005-zt]
...
The value is (not so) different compared to the unweighted version (`r ybar`). They are not exactly the same but given that weights are pretty homogeneous the two estimates are similar.
...
Clearly, as $n$ increases, each study is essentially a perfect estimation of $\theta$ as depicted in the theoretical figure (see slide [-@sec-ee]).
...
Again, we can easily demonstrate this with a simulation. We use the same simulation as slide [-@sec-ee-high-low] but include $\tau^2 = 0.2$.
...
In R there are several packages to conduct a meta-analysis. For me, the best package is `metafor` [@Viechtbauer2010-xz]. The package supports all steps in meta-analysis providing also amazing documentation:
...
filor::show_file(here("chapters/files/power-example.R"), how = "code")
```
...
knitr::include_graphics(here("img/big-picture-pb.svg"))
```
...
- @Orwin1983-vu proposed a new method for calculating the number of studies required to reduce the effect size to a given target
...
The Trim and Fill method [@Duval2000-ym] is used to impute the hypothetical missing studies according to the funnel plot and recompute the meta-analysis effect. Shi and Lin [@Shi2019-pj] provide an updated overview of the method with some criticisms.
...
In R we can use the `metafor::selmodel()` function to implement several types of models. For example, we can apply the Citkowicz and Vevea [-@Citkowicz2017-ox] model:
...
> Hopefully there won't be a need for these models in the future [@Viechtbauer2021-od]
