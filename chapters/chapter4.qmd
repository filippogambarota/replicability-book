...
In earlier chapters we introduced the concept of replication from both statistical and theoretical perspectives. We now focus on analyzing the outcome of replication experiments. Given the many definitions of replication, there are also multiple ways to assess the outcomes of replication studies.

Much of the discussion will focus on replication metrics, an area of statistics which keeps growing. @Heyard2024-hv published an extensive systematic review, supported by an online catalog, and maintains an up-to-date database with more than 50 replication measures organized and classified according to a common scheme.

The database is available at the following link [http://rachelheyard.com/reproducibility_metrics/](http://rachelheyard.com/reproducibility_metrics/). Here we selected methods from the database according to the following criteria:

- we included methods for answering to the question *"Is a study replicated?"* but also methods to combine evidence and estimate the size of the effect without an inferential decision
...
- we included both bayesian and frequentist approaches
- when methods were very similar we focused on the most recent or general version

In general choosing a single measure is probably not the best solution. In fact, large-scale replication projects [e.g., @Collaboration2015-ep; @Errington2021-fb; @The-Brazilian-Reproducibility-Initiative2025-iw] always evaluates the results across different metrics.

## Replication studies as meta-analysis

@Hedges2019-ry points out that replication studies can be seen from a statistical point of view as meta-analyses. Typically there is a single initial study and one or more attempts to replicate its initial findings. For the simple one-to-one replication design we have a meta-analysis with two studies while in the one-to-many design we have a meta-analysis with $R$ studies. When it comes to evaluating the results of the replication studies we can choose between several methods [see @Heyard2024-hv] and some of them are specifically meta-analysis based trying to pool  evidence from original and replication studies. We consider meta-analytic thinking to be crucial to understand the replication studies and methods but not all methods are strictly meta-analyses in the usual sense.

In fact, while the methodology of the initial or replication studies is important when it comes to evaluating a single result, at the aggregated level we evaluate how a certain focal parameter (e.g., the difference between two groups or conditions) vary across replications and if there is evidence, whatever the criteria, for a successful replication. Whatever is the model used in the original studies we can essentially think at the aggregated level without loss of generality.

## Simulating data

While real-world examples are important, simulating simplified examples is a good strategy to understand replication methods from a statistical point of view. Furthermore, simulating data is nowadays considered an important tool to teach and understand statistical methods [@Hardin2015-ss]. Monte Carlo simulations are also necessary to estimate statistical properties (e.g., statistical power or type-1 error rate) of complex models.

For the simulated examples we define the following simulation approach:
...
We put everything in a function that can be used to simulate different scenarios.
...
Let's compute the proportions of replication studies that are statistically significant:
...
Let's imagine an original experiment with $n_{0} = 30$ and $\hat \theta_{0} = 0.5$ that is statistically significant $p \approx 0.045$. Now consider a direct replication study (thus assume $\tau^2 = 0$) with $n_{1} = 350$ which found $\hat \theta_{1} = 0.15$, which is statistically significant as $p\approx 0.047$.
...
The main improvement of confidence/prediction interval methods is taking into account the size of the effect and the precision. The confidence interval represents the sampling uncertainty around the estimated value. It is interpreted as the percentage of confidence intervals under hypothetical repetition of the same sampling procedure that contains the true value.
...
Where $t_{\alpha}$ is the critical test statistics for significance level $\alpha$. This version of the confidence interval is symmetric around the estimated value and the width is a function of the estimation precision.
...
:::{.cons}
- The original study is assumed to provide a reliable estimate
- No extension for *many-to-one* designs
- Original studies with limited precision lead to higher success rate
:::
...
One potential problem of this method is that original studies with wide intervals are easier to replicate. 
...
A similar approach is checking if the original effect size is contained within the replication confidence interval. These methods depends on the precision of studies. Formally:
...
The CI methods described so far are only considering uncertainty from eithernthe original or the replication study. The prediction interval PI expresses the uncertainty of a future observation (in this case a study) rather than the estimated parameter.

If we compute the difference between the original and the replication study, the sampling distribution of the difference represents the range of values that are expected *in absence of other source of variability*. In other terms, assuming that $\tau^2 = 0$.

If the original and replication studies comes from the same population, the sampling distribution of the difference is centered on zero with a certain standard error $\theta_{0} - \theta_{r} \sim \mathcal{N}\left(0, \sqrt{\sigma^2_{\hat \theta_{0} - \hat \theta_{r}}} \right)$ (subscript $0$ is to indicate that the effect is expected to be sampled from the same population as $\theta_{0}$)
...
Another related metric is the $\hat P_{> 0}$, representing the proportion of replications following the same direction as the original effect. Before simply computing the proportions they adjust the estimated $\theta_{r}$ ($r = 1, \dots, R$) with a shrinkage factor:

$$
\tilde{\theta}_{r} = (\theta_{r} - \mu_{\theta_{r}}) \sqrt{\frac{\hat \tau^2}{\hat \tau^2 + \sigma^2_{r}}}
$$

Here $\sigma^2_r$ is the sampling variance for the replication effect size $r$. This method is  similar to the vote counting but we are adjusting the effects with a shrinkage factor based on $\tau^2$.
...
Another approach is to combine the original and replication results using a meta-analysis model. This is applicable to both *one-to-one* and *many-to-one* designs. In this case we can test if the pooled estimate is different from 0 or another practically relevant value.

:::{.pros}
- Uses all the available information, especially when fitting a random-effects model
- Takes into account the precision of individual studies, for example by inverse-variance weighting
:::

:::{.cons}
- Does not consider publication bias
...
where $\bar \theta_R$ is the inverse-variance weighted average, for example from a fixed-effect model. The $Q$ statistics is a weighted sum of squares. Under the null hypothesis where all studies are equal $\theta_0 = \theta_1, ... = \theta_R$ the $Q$ statistics has a $\chi^2$ distribution with $R - 1$ degrees of freedom. Under the alternative hypothesis the distribution is a non-central $\chi^2$ with non centrality parameter $\lambda$. The expected value of the $Q$ is $E(Q) = \nu + \lambda$, where $\nu$ are the degrees of freedom.
...
- In case of an **extension**, $\lambda < \lambda_0$ where $\lambda_0$ is the maximum value considered equivalent to zero. In other words, $\lambda_0$ is the threshold above which the variability is too high to be considered negligible for a replication [in @Machery2020-sv terms].

This approach is testing the consistency (i.e., homogeneity) of replications. A successful replication should minimize the heterogeneity and the presence of a significant $Q$ statistics would suggest a failed replication attempt. @Hedges2019-pr and @Mathur2019-vh discuss extensively this approach highliting some important points:

- facing different replication setup (burden of proof on replicating vs non-replicating, many-to-one and one-to-one, etc.)
- interpreting and choosing an appropriate $\lambda_0$ parameter
- evaluating the power and statistical properties of replication metrics

When evaluating a replication we can use the `Qrep()` function that calculates the p-value based on the Q sampling distribution.
...
#### Q Statistics for an extension study

In case of an extension we need to set $\lambda_0$ to a meaningful value but the overall test is the same. The critical $Q$ is no longer evaluated with a central $\chi^2$ but is now a non-central $\chi^2$ with $\lambda_0$ as non-centrality parameter.
...
Having introduced the $I^2$ statistics we can derive a $\lambda_0$ based on $I^2$. @Schmidt2014-kw proposed that when $\tilde{v}$ is at least 75% of total variance $\tilde{v} + \tau^2$ thus $\tau^2$ could be considered negligible. This corresponds to a $I^2 = 25%$ and a ratio $\frac{\tau^2}{\tilde{v}} = 1/3$ thus $\lambda_0 = \frac{(R - 1)}{3}$ can be considered negligible heterogeneity.
...
- the replication study found an effect of $\theta_{r} = 0.2$ with $n = 100$ subjects
...
Let's consider a (not very innovative :smile:) example. We need to evaluate the fairness of a coin. The parameter is the probability $\pi$ of success (i.e., head). We have our prior belief about the coin (e.g., fair but with some uncertainty). We toss the coin $k$ times and we observe $x$ heads. What are my conclusions?

$$
p(\pi|D) = \frac{p(D|\pi) \; p(\pi)}{p(D)}
$$
where $\pi$ is our parameter and $D$ the data. $p(\pi|D)$ is the posterior distribution that is the normalized product between the likelihood $p(D|\pi)$ and the prior $p(\pi)$. $p(D)$ is the probability of the data (aka marginal likelihood) and is necessary only for the posterior to be a proper probability distribution.
...
The SDR is a convenient shortcut to calculate the Bayes Factor [see @Wagenmakers2010-fj; @Marin2010-oh]. The idea is that the ratio between the prior and posterior density distribution for the $H_1$ is an estimate of the Bayes factor calculated in the standard way.
...
When there are multiple parameters in play and the null hypothesis is expressed in terms on one of them only, this representation is valid only under the condition that the prior for any of the other parameters under the null is the same as the conditional prior for those parameters under the alternative. 
...
::: {.callout-note}
The assumption of @Verhagen2014-tx is that the original study performed a Bayesian analysis with a completely flat prior. Thus the confidence interval is approximately the same as the Bayesian credible interval.
:::
